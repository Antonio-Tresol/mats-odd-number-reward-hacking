...
* What I did: Made the repository public so the trace explorer can be served from GitHub Pages and linked from Antonio's site. Before flipping visibility, four files left the public tree for the gitignored archive at data/mats/notes/: the admissions document export and its image, the synthesis of the mentor's papers, and the SPAR take-home text; AGENTS.md, the application brief, and the archive README now point there. The README's status and findings paragraphs were brought up to the falsification pass and the Fable probe, and it links the explorer. A copy-and-deploy workflow (.github/workflows/deploy-pages.yml) publishes the committed explainer as the repository's Pages site. No experiment ran and no tree node changed. Rewrote the history so the five files that left the tree this morning are gone from every commit as well: the admissions document export and its image, the synthesis of the mentor's papers, the SPAR take-home prompt with its embedded screenshot, and the paraphrase of it. git filter-repo removed the paths from all 100-odd commits and the result was force-pushed; the SPAR repository, which shares the two take-home files, was rewritten the same way later in the day, after the session's permission mode blocked the first attempt and Antonio told the session to run it (its head is now 65ab4f4). Every commit hash changed, so the record's three pointers to commits were repointed by filter-repo's own map: the provenance pin in both falsification scorecards and the falsification note (82e0bcd is now 3b6e2da) and the handoff note's pointer to the retired write-up draft (aa0843c is now 4cbac3d). The two log entries that named the old paths carry a bracketed note saying where the files went. No experiment ran and no tree node changed. Deleted the explainer's Findings view: the nine places-to-start cards and the per-model syntheses it showed. Antonio found the page incomprehensible beside the write-up's story, which is about qwen3.8-27b alone, and the cards ranged over Kimi, the misread labels and traces that say ChatGPT. The syntheses stay under notes/trace-syntheses/ as evidence for Q1.H7.E4; the page no longer embeds them, so the builder's synthesis loader, the --syntheses-dir flag, the markdown dependency and the test fixture that fed the view went with it, and the page shrank from 13.9 MB to 13.3 MB. Renamed the repository to odd-number-reward-hacking and rewrote its public face. Antonio did not submit the application the fork had been framed around, so the name, the package description, the GitHub description and the agent instructions drop that framing; notes/mats/ and data/mats/ stay as record. The README is new: a visitor's page modelled on the emotion-vectors repository, with the gaming-rate figure as its hero, three findings, the confusion and clarification figures with one caption each, install, method, quick start, reproduction steps, a data table, caveats, a citation file and an MIT licence. The two figures it used to carry, the length confound and the branch curves, are gone from it; every number it keeps traces to Q1.H1.E1.C1, Q1.H1.E4.C1, Q1.H7.E2.C9 to C13, Q1.H7.E6.C1 to C2 and Q1.H7.E7.C1 to C2. The site card and the Pages workflow point at the new name.
* What I expected vs what happened: The workflow's first run failed at the deploy step with a 404 and the message to enable Pages, which is the failure it was written to have on a private repository; the copy step before it succeeded, so the workflow is right and only the visibility was in the way. Those four files stay in git history: the take-home text is already in the public SPAR repository's history and the admissions document is a public Google Doc, so going public adds no exposure the SPAR repository did not already carry. The rewrite was clean: filter-repo reports zero copies of any purged path in any ref, and the pin check passes because pins hash the evidence files, never the scorecards that carry the commit field. GitHub keeps unreachable commits fetchable by hash for a while after a force-push, so the old hashes may still resolve there until its garbage collection runs or support purges them on request. The rail loses one button and every other view is unchanged. GitHub redirects the old repository URL; the Pages site does not redirect, so the workflow was re-run to publish under the new name and the card on Antonio's site was repointed.
* What this changes about my thinking: The record is now the public front door as well as the working record, so its README has to say what the falsification pass established rather than what the SPAR take-home left open. The archive rule holds: raw sources and anything shared in confidence stay local and gitignored, and only notes derived from them are committed. Nothing about the findings. The archive rule now covers history too: a file shared in confidence is kept out of the tree from the first commit, because removing it later means rewriting every hash the record pins. No experiment ran and no tree node changed. The repository is the public front door, so its README speaks to a visitor and the record speaks to whoever works in it.
* What I will do next: Enable Pages with the GitHub Actions source, run the workflow, confirm the explorer URL serves, then push the site card that links to it. The MATS write-up is unaffected by any of this. Refresh any clone with a fetch and reset, since main was force-pushed. The MATS write-up is unaffected. Cut the README's findings paragraphs to the write-up's story the same way if Antonio wants them to match. Decide whether the SPAR repository, whose content this one contains in full, is archived with a pointer here.
...
* What I expected vs what happened: Antonio expected Fable to answer even, and it did, 3 of 3 in each arm, every answer 42. Its conflict-arm reasoning is about 250 visible characters and calls the grader block an injection attempt or a hidden instruction to game the system, then ignores it. Two things to hold onto: the visible reasoning is shorter than the reasoning tokens billed (95 to 143 tokens against about 60 visible), so it is most likely a summary rather than the chain of thought itself; and the pin was sent as the alias and served as `anthropic/claude-fable-5.1-20260831`, so the pin now names the dated snapshot for any later run. Effort high changed nothing: 42 three times, 88 to 171 reasoning tokens, the model declining to use the budget. One of the three reads the block as a test of whether it would game the grader, which is the eval-awareness reading beside the injection reading.
* What this changes about my thinking: Three rollouts settle nothing about a rate, but they show a fourth way to read the block beside gaming, silent compliance and disclosure: naming it as an injection. Where the open-weights models argue about who the real principal is, Fable treats the second party as an attacker. Whether that difference is training or size is not a question this project can afford to ask. Fable's default reasoning is already its full answer to this prompt; effort is not the lever. The reference point for the write-up is one line: a frontier closed model reads the block as an attack or a test and answers even, six of six across two settings.
* What I will do next: No further Fable collection unless Antonio asks; the budget concern is his. The write-up may mention the probe as a one-line reference point with n = 3 stated. Nothing further on Fable. The write-up proceeds.

### 2026-09-01

* What I did: Forked the repository for the application to Neel Nanda's MATS 12.0 stream. The full history was cloned into a new private GitHub repository, mats-odd-number-reward-hacking, standing beside the SPAR repository rather than inside the application workspace, with the SPAR repository kept as the upstream remote. The admissions document and Antonio's synthesis of the mentor's last year of papers (both now `data/mats/notes/`, gitignored, and out of the history since 2026-09-08), and the matching bibliography (`references/neel.bib`) were copied in. A new note, notes/mats/application-brief.md, quotes with line numbers every rule in the admissions document that binds this project: the deadline, what the 20 hours plus 2 include, the executive summary format, the rule on work done before the application, and how the write-up is judged. The SPAR framing in the agent instructions, the package description, the README, and this file's project summary was rewritten for the new setting. No experiment ran and no tree node changed. Ran the falsify gate on the twenty claims the write-up rests on, the first pass on any claim. Antonio reported his own active time at five hours or less and put his SPAR write-up forward as the document to test, so the tests started from its claims. `uv run odd-number falsify` (`src/odd_number/falsification/`, seed 20260901, 20,000 permutations, no API calls) recounts every stated cell with its own parser and judge-sidecar lookup, then runs Fisher, Wilson, Holm, bootstrap and permutation tests, a logistic fit of odd on log reasoning length with a length-matched band as the model-free companion, a length-stratified permutation for the confusion labels, a constant-rate null for the per-sentence sawtooth, and the two-story tests on the cross-prompt sweep; the scorecard is `results/falsify-scorecard-2026-09-01.json`. `Q1.H7.E4.C1` was tested by reading: twelve of the 74 odd answers drawn with a seed, read by two agents blind to the project under a protocol registered first, 118 quotes all resolving, in `results/falsify-verification-Q1.H7.E4.C1-2026-09-01.json`. Added `Q1.H7.E7` with two claims, because the write-up's confusion table (built 2026-08-30 in four commits and never logged) had no node, and `Q1.H7.E6.C3`. Fifteen claims survived, five weakened, none failed; the note is `notes/falsification-2026-09-01.md`.
* What I expected vs what happened: I expected the admissions document to treat the SPAR work as prior research, which it says it judges more harshly than a normal application. It does something more useful: work done on one's own in 20 hours or fewer and not for the application can be treated as a normal application project, so the clock started with the SPAR hours and the remaining budget continues the same investigation. One difference surfaced in the accounting. The SPAR rules excluded replication from their limit; the MATS rules count every hour of active work on the project, so the hour estimate the application needs must include the replication runs. Antonio expected the record to hold, and on the numbers it did: 38 of 38 stated counts reproduce, no odd answer anywhere was decided by the judge, no duplicate rule moves a count, and every row was served by its pin. What did not hold was three interpretations. The paraphrase claim's upward moves are what a constant low rate per model produces across 29 cells, and only Kimi's downward move survives correction. The post's label zeros, on Kimi and on qwen3.8, are what the collapse in deliberation predicts on its own, so `Q1.H1.E6.C1` and `C2` are weakened, while the affirming sentence's zero is not: a length-only fit expects 4.3 odd among user_authored's traces against 0 observed, and in the band of lengths those traces span, 10 of 57 plain-prompt traces answered odd against 0 of 40. The per-sentence sawtooth's largest step is what a constant rate gives 29% of the time. The reading found the principal argument in 12 of 12 sampled odd answers and a stated reward preference beside it in at least 3, which the claim had excluded. Two things I did not expect: within the baseline conflict arm, odd answers are not longer than even ones (permutation p = 0.65), and of the six confusion questions only whether the sender is a person or a machine predicts an odd answer once length is controlled (stratified p = 0.008, adjusted p = 0.026); the other five are length.
* What this changes about my thinking: The question does not change, and the mentor's own problem list describes this setting: read the chain of thought to form hypotheses, test them with precise counterfactual changes, and take special interest in cases where that is not enough and resampling or internals are needed, which is where Q1.H8 already sits. What changes is the deliverable. Antonio writes the application form answers and the executive summary in his own voice; the executive summary is at most 600 words with graphs; randomly selected raw examples follow it; and the write-up states what was verified and how. The audit trail this record keeps is the evidence for that last requirement, and the write-up says plainly that this is the SPAR take-home extended under the MATS budget. The write-up's phenomenon stands and its mechanism sentence needs splitting. That clarifying the confusion stops the behaviour is true of every rate; as a claim about what the clarification does, it holds for the affirming sentence by two routes (the length test here and the cross-prompt resampling) and is not separable from shorter deliberation for the post's labels. Confused rather than reward seeking is not what the traces say: in this sample the principal argument and the reward preference sit side by side. The length figure shows that the arms differ, not that the answers do. The confusion table's one load-bearing row is the sender question. On the record itself, the falsification that was still owed is now on every claim the executive summary will use. The reader runs on `Q1.H7.E4.C1` disagree (survives against weakened) and the verdict is weakened because the dissenting reader's three reward-preference quotes resolve verbatim in the results files, while the agreeing reader's coding rule assigned principal whenever both moves appeared and so could not have surfaced them.
* What I will do next: Antonio estimates the hours spent so far and decides how the remaining budget is split between the open falsification steps, the length confound that Q1.H8 leaves tied, and the write-up. The first working session in the fork runs the verifier, reads the brief, and picks the one experiment that most changes what the executive summary can claim. Antonio reads the note and the two scorecards, checks the three reward-preference quotes and the sender-question table himself, and decides whether `Q1.H7.E6.C3` and `Q1.H7.E7.C2` go into the executive summary as they stand or need a pass of their own. Then the write-up: agents draft the technical body from the tree, the seeded random-example draw, and the figure check; he writes the summary and the form on the separate two-hour clock. The next-turn probe, `Q1.H6.E3`, is still running and should be graded or abandoned before submission. Nothing from today is committed yet.
...
* What I did: Scaffolded the project from the research-harness — skills, research tree, this log, and the mechanical validator — and wrote Q1 into TREE.md. Merged the separate take-home workspace repo into this one under `notes/take-home/` (moved to `data/mats/notes/` and out of the history on 2026-09-08), retiring the second repo. Began the literature pass with Singh et al., *Model Forensics* (arXiv:2606.26071) — the mentor's own paper, which defines the protocol this take-home asks us to apply — took verbatim notes in `notes/model-forensics-paper-notes.md`, and derived a `model-forensics` skill from those notes. Fixed two Windows path-separator bugs in the vendored lanorme plugins that were silently disabling PROV-003 and STALE-002.
